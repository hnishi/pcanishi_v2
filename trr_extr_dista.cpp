...
/* extr_dista
 * extract distance section 
 */
   cout<<"\n--- DISTANCE SECTION --- \n";
   unsigned int numofatom = vec_tar.size()/3;
   unsigned int frame = vec.size() / vec_tar.size();
   cout<<"The number of atoms = "<<numofatom<<endl;
   cout<<"Thr number of structures is "<<frame<<endl;
   cout<<"The number of distances between the atoms = "
       <<(numofatom*(numofatom -1))/2<<endl;

   cout<<"\n now calculating"<<endl;
   vector<double> dista;
   for(unsigned int n=0;n<frame;n++){
      for(unsigned int i=0;i<numofatom;i++){
         for(unsigned int j=i+1;j<numofatom;j++){
            double x = vec[n*numofatom*3 + j*3] - vec[n*numofatom*3 + i*3];
            double y = vec[n*numofatom*3 + j*3 +1] - vec[n*numofatom*3 + i*3 +1];
            double z = vec[n*numofatom*3 + j*3 +2] - vec[n*numofatom*3 + i*3 +2];
            dista.push_back( sqrt(x*x + y*y + z*z) );
	 }
      }
   }
   cout<<"The number of calculated distance = "<<dista.size()<<endl;
   cout<<"The number of calculated distance / structure = "<<dista.size()/frame<<endl;
   cout<<"DEBUG: dista[final] = "<<dista[dista.size() -1]<<endl;

   if(dista.size()/frame != (numofatom*(numofatom -1))/2 ){
      cout<<"ERROR: program ended in DISTANCE SECTION"<<endl;
      return 2;
   }
 ...
 * */
   ...
   unsigned int dim_Q = dista.size()/frame ;
